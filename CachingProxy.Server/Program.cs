...
{
    var options = builder.Configuration.GetSection(CachingProxyOptions.SectionName).Get<CachingProxyOptions>() ?? new CachingProxyOptions();
...
app.MapGet("/proxy", async (string url, HttpResponse response, CancellationToken cancellationToken, CachingProxy.Server.CachingProxy cachingProxy) =>
{
    ...
    if (proxyResponse.ContentLength.HasValue)
    {
        response.ContentLength = proxyResponse.ContentLength.Value;
    }
...
{
    var options = config.GetSection(CachingProxyOptions.SectionName).Get<CachingProxyOptions>() ?? new CachingProxyOptions();
    return new
    {
        CacheDirectory = options.CacheDirectory,
        MaxCacheFileSizeMB = options.MaxCacheFileSizeMB,
